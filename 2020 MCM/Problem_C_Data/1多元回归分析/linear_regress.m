...
x123456 = [hairdryerS2(:,[2:5]),hairdryerS2(:,[7:8])]; % Extract observed value x
% Y = microwaveS1(:,[6]);
% x123456 = [microwaveS1(:,[2:5]),microwaveS1(:,[7:8])];
% Y = percifierS1(:,[6]);
%x123456 = [percifierS1(:,[2:5]),percifierS1(:,[7:8])];
Size = size(x123456)
X=[ones(Size(1),1),x123456] % Construct data matrix of multiple linear regression analysis
[beta,betarint,r,rint,st] = regress(Y,X) % Calculate regression coefficients and stats values
...
fw1 = finv(0.025,m,n-m-1) % Quantiles,if fw1<F<fw2 then model is not allowed
fw2 = finv(0.975,m,n-m-1)
c = diag(inv(X'*X)) % Calculate c_jj and stats value t
t = beta./sqrt(c)/sqrt(q/(n-m-1))
tfw = tinv(0.975,n-m-1) % Quantiles,if t_i<tfw<t_j then t_0...t_i can be abandoned
rcoplot(r,rint)
